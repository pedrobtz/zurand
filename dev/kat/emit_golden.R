...
key <- rng_key(42L, engine = "philox4x64")
keys <- rng_key(42L, n = 3L, engine = "philox4x64")
...
"test_that(\"rng_uniform() is bit-stable\", {",
  blk("rng_uniform(rng_key(42L, engine = \"philox4x64\"), 12L)", g(rng_uniform(key, 12L))),
"",
  blk("rng_uniform(rng_key(42L, engine = \"philox4x64\"), 4L, min = -2, max = 5)",
      g(rng_uniform(key, 4L, min = -2, max = 5))),
"})",
"",
"test_that(\"rng_normal() is bit-stable, including the ziggurat slow paths\", {",
  blk("rng_normal(rng_key(42L, engine = \"philox4x64\"), 12L)", g(rng_normal(key, 12L))),
"",
...
"  # pin the wedge and tail branches that most of the C code exists to serve.",
  ...
  blk("rng_normal(rng_key(42L, engine = \"philox4x64\"), 200000L)[tail_idx]", g(z[tail_i])),
"",
  blk("rng_normal(rng_key(42L, engine = \"philox4x64\"), 4L, mean = 2, sd = 3)",
      g(rng_normal(key, 4L, mean = 2, sd = 3))),
"})",
"",
"test_that(\"rng_integer() is bit-stable, including the rejection path\", {",
  blk("rng_integer(rng_key(42L, engine = \"philox4x64\"), 12L, min = 1L, max = 6L)",
      paste0(rng_integer(key, 12L, min = 1L, max = 6L), "L")),
"",
"  # A near-maximal range makes Lemire rejection actually fire.",
  blk("rng_integer(rng_key(42L, engine = \"philox4x64\"), 8L, min = 1L, max = 2000000000L)",
...
"  # this is the case that must not be computed as min + (int)offset.",
  blk("rng_integer(rng_key(42L, engine = \"philox4x64\"), 8L, min = -2000000000L, max = 2000000000L)",
      paste0(rng_integer(key, 8L, min = -2000000000L, max = 2000000000L), "L")),
"})",
"",
"test_that(\"rng_fold() derives stable keys\", {",
  blk("format(rng_fold(rng_key(42L, engine = \"philox4x64\"), \"layer1\"))",
      format(rng_fold(key, "layer1")), quote = TRUE),
  blk("format(rng_fold(rng_key(42L, engine = \"philox4x64\"), 7L))",
...
"  # as well as the values.",
  blk("as.vector(rng_normal(rng_key(42L, n = 3L, engine = \"philox4x64\"), 4L))",
...
"  expect_identical(attr(fx, \"engine\"), \"xoshiro256pp\")",
  blk("format(rng_fold(rng_key(42L, engine = \"xoshiro256pp\"), \"layer1\"))",
      format(rng_fold(xkey, "layer1")), quote = TRUE),
"})",
"",

...
`Authors@R` and `License` in `DESCRIPTION` are still scaffold placeholders and will fail or warn in `R CMD check` until filled in (e.g. `usethis::use_mit_license()`). Two vendored components are BSD 3-clause and their copyright holders must be credited (cph roles in `Authors@R` or `inst/COPYRIGHTS`) before release: Random123 (D. E. Shaw Research, license at `src/Random123/LICENSE`) and NumPy's ziggurat tables (NumPy Developers, license at `src/numpyzig/LICENSE`).

...
`zurand` provides stateless random numbers for R, built on the Philox4x64-10 counter-based generator from the vendored **Random123** library. Every value is a pure function of an immutable key plus sampler arguments; ordinary samplers never read or mutate `.Random.seed`.
